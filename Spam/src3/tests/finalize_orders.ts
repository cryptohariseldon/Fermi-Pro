...
async function finalizeEvents() {
  ...
  const secretKey = JSON.parse(
    fs.readFileSync('/Users/dm/.config/solana/id.json'),
  );
  ...
  if (market != null) {
    ...
    const makerAtaPublicKey = await checkOrCreateAssociatedTokenAccount(
      provider,
      market.baseMint,
      makerpubkey,
    );
    const takerAtaPublicKey = await checkOrCreateAssociatedTokenAccount(
      provider,
      market.quoteMint,
      makerpubkey,
    );
