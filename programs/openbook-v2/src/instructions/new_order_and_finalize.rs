...
pub fn market_order_and_finalize(
    ...
    let matchedorder = find_by_key(order_id)

    require!(matchedorder.qty >= qty, "provided order has insuffient qty");
    
    // update matchedorder in place to new qty after debiting this trade qty
    matchedorder.quantity -= qty;

    // remove by key if matchorder.qty is 0
    if matchedorder.quantity == 0 {
        market.orders.remove_by_key(&order_id);
    }
