...
async function placeOrder(): Promise<void> {
  ...
  const openOrders = new PublicKey("9p8QkDc3eMacdjGwRNgLm9dt7k1JRGjViDry2VYmKiLt");
  const openOrder  = await client.deserializeOpenOrderAccount(new PublicKey(openOrders.toString()));
  //console.log("eventQ: ", eventQ);  
  if (openOrder !== null) {
    console.log("openOrder details: ", openOrder.toString());

    const event1 = openOrder[0];
